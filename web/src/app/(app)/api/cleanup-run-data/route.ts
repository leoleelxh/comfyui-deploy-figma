...
async function autoCleanupOldData(): Promise<void> {
  try {
    ...
    const oldRuns = await db.query.workflowRunsTable.findMany({
      where: eq(workflowRunsTable.status, "success") // 只清理成功的运行记录
    });

    // 过滤出创建时间超过2小时的记录
    const runsToClean = oldRuns.filter(run => 
      run.created_at && new Date(run.created_at) < cutoffTime
    );

    console.log(`找到${runsToClean.length}条需要自动清理的记录`);

    // 批量处理旧记录
    for (const run of runsToClean) {
      await cleanupOutputData(run.id);
      await cleanupInputData(run.id);
    }

    console.log('自动清理完成');
  } catch (error) {
    console.error('自动清理过程中出错:', error);
  }
}
...
export async function POST(request: Request) {
  try {
    ...
    // 触发自动清理（不等待完成）
    Promise.resolve().then(() => autoCleanupOldData());
